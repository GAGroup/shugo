...
public class VideoUploadeAction extends ActionSupport   {
	/**
	 * 
	 */
    ...
    private String videoFileName; //文件名称kb12.png(包括扩展名)
    private String videoContentType; //文件类型--需要合法验证 image/png
    
    //表单中要保存文件的信息
    private Integer videoType;//分类的类别
     ...
     * @throws Exception
     */
    public String uploadeVideo()throws Exception{
    	//生成videoInfo规则
    	if (video != null||!visitPath.trim().isEmpty()) {
    		VideoInfo videoInfo = Action2VideoInfo();
     ...
     * @return
     */
	private VideoInfo Action2VideoInfo() {
        ...
        if(visitPath.trim().isEmpty()){
        	//得到保存的网络路径---上传到服务器
        	String path = userPath;
        	videoInfo.setVisitPath(path+"/"+holdVideoName);
        }else{
        	//校验是否为合法路径--外网视频
        	videoInfo.setVisitPath(visitPath.trim());
        }
