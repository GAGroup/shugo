...
public class LoginedCheckInterceptor extends AbstractInterceptor {
	...
	public String intercept(ActionInvocation ai) throws Exception {
		//取得请求的URL
		String url = ServletActionContext.getRequest().getRequestURL().toString();					 //获取当前url
		String prim = null;
		//Admin admin = null;
		int index = 0;
		//验证Session是否过期
		if(!ServletActionContext.getRequest().isRequestedSessionIdValid()){
			...
			//对登录与注销请求直接放行,不予拦截
			if (url.indexOf("admin_login.action")!=-1 || url.indexOf("admin_logout.action")!=-1){     //查找url中是否有指定字符串
				return ai.invoke();
			}else{
